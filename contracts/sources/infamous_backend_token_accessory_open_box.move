...
/// InfamousBackendTokenAccessoryOpenBox used to mutate accessory nft's property by authed account 
module infamous::infamous_backend_token_accessory_open_box {
    ...
    const EACCESSORY_BOX_ALREADY_OPENED: u64 = 2;
    ...
    /// open box, mutate infamous accessory nft with certain properties.
    public entry fun open_box(sender: &signer, accessory_token_name: String, name: String, kind: String, gender: String, attributes: String,) acquires OpenBoxStatus {
        ...
        // check opened
        assert!(!is_box__opened(token_id), error::invalid_state(EACCESSORY_BOX_ALREADY_OPENED));
    ...
    /// update infamous accessory token open status
